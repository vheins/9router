...
import { tableInfo, mapColumnType } from "./dialect.js";
import { getDbMode, openSqliteAdapterReadOnly } from "./driver.js";

// Marker file: prevents re-importing legacy JSON when user wipes data.sqlite.
const MIGRATED_MARKER = path.join(DB_DIR, ".migrated-from-json");
// Marker file: prevents re-copying SQLite → MariaDB on every boot.
const MARIA_MIGRATED_MARKER = path.join(DB_DIR, ".migrated-to-mariadb");

// Rows per bulk-insert batch during SQLite → MariaDB copy.
...
function sqlMode() {
  return getDbMode() === "mariadb" ? "mariadb" : "sqlite";
}
...
async function importWithAssertion(adapter, tableName, rows, insertFn, rowMeta) {
  ...
  if (Number(inserted) !== rows.length) {
...
async function runVersionedMigrations(adapter) {
  ...
  await adapter.exec(buildCreateTableSql("_meta", TABLES._meta, mode));

  const current = parseInt(await getMetaFor(adapter, "schemaVersion", "0"), 10) || 0;
  ...
  for (const m of pending) {
    await adapter.transaction(async () => {
      await m.up(adapter);
      await setMetaFor(adapter, "schemaVersion", m.version);
    });
...
async function syncSchemaFromTables(adapter) {
  ...
  for (const [tableName, def] of Object.entries(TABLES)) {
    ...
    await adapter.exec(buildCreateTableSql(tableName, def, mode));

    // Diff columns (PRAGMA on SQLite, information_schema on MariaDB)
    const infoSql = mode === "mariadb" ? tableInfo(tableName) : `PRAGMA table_info(${tableName})`;
    const existing = await adapter.all(infoSql);
    const existingNames = new Set(existing.map((r) => r.name));
    for (const [colName, colDef] of Object.entries(def.columns)) {
      if (!existingNames.has(colName)) {
        // ADD COLUMN restrictions: no PRIMARY KEY / UNIQUE at add time.
        // We strip PRIMARY KEY / UNIQUE since those are only valid at create time.
        const stripped = colDef
          .replace(/PRIMARY KEY( AUTOINCREMENT)?/i, "")
          .replace(/UNIQUE/i, "")
          .trim();
        const safeDef = mode === "mariadb" ? mapColumnType(stripped, { indexed: false }) : stripped;
        try {
...
async function importLegacyUsage(adapter, data) {
  ...
  if (typeof data.totalRequestsLifetime === "number") {
    await setMetaFor(adapter, "totalRequestsLifetime", data.totalRequestsLifetime);
  }
}
...
// ─── SQLite → MariaDB one-time copy ──────────────────────────────────────
// Runs when: DB_MODE=mariadb AND target MariaDB is fresh (empty _meta) AND a
// SQLite file exists AND the marker is absent. The SQLite file is only READ.
async function migrateSqliteToMariaDb(mariaAdapter, { force = false } = {}) {
  if (!force && fs.existsSync(MARIA_MIGRATED_MARKER)) return false;
  if (!fs.existsSync(DATA_FILE)) return false;

  const t0 = Date.now();
  let src;
  try {
    // Opened strictly READ-ONLY: no mutating PRAGMAs (journal_mode=WAL),
    // no WAL checkpoint timer, no shutdown handlers → source file untouched.
    src = await openSqliteAdapterReadOnly(DATA_FILE);
  } catch (e) {
    console.warn(`[DB][migrate] SQLite → MariaDB: cannot open source SQLite (${e.message}) — skipping`);
    return false;
  }

  try {
    // 1. Ensure schema exists in MariaDB before copying (dialect-aware DDL).
    for (const [name, def] of Object.entries(TABLES)) {
      await mariaAdapter.exec(buildCreateTableSql(name, def, "mariadb"));
      for (const idx of def.indexes || []) {
        try { await mariaAdapter.exec(idx); } catch {}
      }
    }

    // 2. Copy every table's rows verbatim (ids + all columns preserved).
    let tablesCopied = 0;
    let rowsCopied = 0;
    await mariaAdapter.transaction(async () => {
      for (const name of Object.keys(TABLES)) {
        let rows;
        try { rows = await src.all(`SELECT * FROM ${name}`); } catch { continue; }
        if (!rows || rows.length === 0) { tablesCopied++; continue; }
        ...
        for (let i = 0; i < rows.length; i += COPY_CHUNK) {
          ...
          for (const row of chunk) {
            await mariaAdapter.run(insertSql, cols.map((c) => (row[c] === undefined ? null : row[c])));
          }
        }
        tablesCopied++;
    ...
    try { fs.writeFileSync(MARIA_MIGRATED_MARKER, new Date().toISOString()); } catch {}
    console.log(`[DB][migrate] SQLite → MariaDB: copied ${tablesCopied} tables (${rowsCopied} rows) in ${Date.now() - t0}ms`);
    return true;
  } catch (e) {
    console.error(`[DB][migrate] SQLite → MariaDB copy failed: ${e.message}`);
    return false;
  } finally {
...
export async function runMigrationOnce(adapter) {
  ...
  let fresh = await isFreshDb(adapter);

  // Prune stale backups every boot so old oversized backups shrink to KEEP.
  pruneOldBackups();

  // 0. MariaDB mode: one-time auto-migration from an existing SQLite file.
  //    Must run BEFORE schema bootstrap below so we copy into a clean DB.
  if (mode === "mariadb" && fresh) {
    const force = process.env.DB_MIGRATE_FORCE === "1" || process.env.DB_MIGRATE_FORCE === "true";
    const copied = await migrateSqliteToMariaDb(adapter, { force });
    if (copied) fresh = await isFreshDb(adapter); // re-evaluate (now populated)
  }
  ...
  if (fresh && hasLegacy && !alreadyImported) {
    ...
    pruneOldBackups();
    console.log(`[DB][migrate] JSON → ${mode === "mariadb" ? "MariaDB" : "SQLite"} in ${Date.now() - t0}ms | legacy JSON kept at DATA_DIR | backup: ${backupDir}`);
    return;
  }
  ...
  const oldVer = await getMetaFor(adapter, "appVersion", null);
  if (oldVer !== newVer) await setMetaFor(adapter, "appVersion", newVer);
}
