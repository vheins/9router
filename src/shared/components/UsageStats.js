...
export default function UsageStats({
  ...
  // Layered router -> combo -> provider -> model graph (1h window, fail-open).
  // Fetched once; live active-path animation rides the existing SSE stream.
  useEffect(() => {
    let cancelled = false;
    fetch("/api/usage/topology")
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => {
        if (
          !cancelled &&
