...
export async function handleForcedSSEToJson({ providerResponse, sourceFormat, targetFormat, provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, customToolNames, trackDone, appendLog, reqTag, log }) {
  ...
  if (isCodexResponsesApi) {
    try {
      ...
      const sseJsonLatency = { ttft: Date.now() - requestStartTime, total: Date.now() - requestStartTime };
      saveUsageStats({ provider, model, tokens: usage, connectionId, apiKey, endpoint: clientRawRequest?.endpoint, latency: sseJsonLatency, silent: true });
      if (log?.line) log.line(reqTag, "📊", formatDoneLine({ usage, latency: sseJsonLatency }));
  ...
  try {
    ...
    appendLog({ tokens: usage, status: "200 OK" });
    const sseJsonLatency2 = { ttft: Date.now() - requestStartTime, total: Date.now() - requestStartTime };
    saveUsageStats({ provider, model, tokens: usage, connectionId, apiKey, endpoint: clientRawRequest?.endpoint, latency: sseJsonLatency2, silent: true });
    if (log?.line) log.line(reqTag, "📊", formatDoneLine({ usage, latency: sseJsonLatency2 }));
